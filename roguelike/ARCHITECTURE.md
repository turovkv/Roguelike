...
- Класс `Enemy` хранит в себе информацию о враге, его координаты стратегии и тд
...
- Для герерации мапы используется класс `FieldBuilder`, который генерирует карту в завсимости от `EnemyFactory` и параметров ширины, высоты и файла с картой. Внутри себя `FieldBuilder` использует интерфейс `FieldGenerator` от которого наследуются следующие классы
